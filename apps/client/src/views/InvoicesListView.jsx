  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Invoices & Billing Ledger</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Track customer invoices, due dates, and payment reconciliations</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
              {filteredInvoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                  <td className="py-3.5 px-4 font-bold text-indigo-600 dark:text-indigo-400 font-mono">{inv.invoice_number}</td>
                  <td className="py-3.5 px-4 text-slate-500 dark:text-slate-400">{inv.quote_number}</td>
                  <td className="py-3.5 px-4 font-medium text-slate-900 dark:text-slate-100">{inv.customer_name}</td>
                  <td className="py-3.5 px-4 text-slate-600 dark:text-slate-300">{inv.due_date}</td>
