  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Deal Health & Anomaly Engine</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Autonomous detection of stalled quotes, margin slippages, and anomalies</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-rose-200 dark:border-rose-500/30 bg-rose-50/50 dark:bg-rose-950/10">
          ...
          <div className="mt-4 flex items-baseline justify-between">
            <span className="text-3xl font-extrabold text-slate-900 dark:text-white">{stalledCount}</span>
            <Badge variant="danger">Escalation Required</Badge>
          </div>
        </Card>

        <Card className="border-amber-200 dark:border-amber-500/30 bg-amber-50/50 dark:bg-amber-950/10">
          <div className="flex items-center justify-between">
          ...
          <div className="mt-4 flex items-baseline justify-between">
            <span className="text-3xl font-extrabold text-slate-900 dark:text-white">{anomalyCount}</span>
            <Badge variant="warning">Margin Slippage</Badge>
          </div>
        </Card>

        <Card className="border-indigo-200 dark:border-indigo-500/30 bg-indigo-50/50 dark:bg-indigo-950/10">
          <div className="flex items-center justify-between">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-700 dark:text-slate-300">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
              {alerts.map((al) => (
                <tr key={al.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                  <td className="py-3.5 px-4 font-bold text-indigo-600 dark:text-indigo-400">{al.quote_number}</td>
                  <td className="py-3.5 px-4 font-medium text-slate-900 dark:text-slate-100">{al.customer_name}</td>
                  <td className="py-3.5 px-4">
                    <Badge variant={al.alert_type === 'Stalled Deal' ? 'danger' : 'warning'}>{al.alert_type}</Badge>
                  </td>
                  <td className="py-3.5 px-4 text-xs text-slate-600 dark:text-slate-300 max-w-xs">{al.detail}</td>
                  <td className="py-3.5 px-4 text-xs text-slate-500 dark:text-slate-400">{new Date(al.flagged_at).toLocaleDateString()}</td>
                  <td className="py-3.5 px-4">
